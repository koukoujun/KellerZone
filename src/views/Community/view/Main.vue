    ...
    <div class="content" >
      <div class="container">
        ...
        <div class="main" v-for="(item,index) in dataList" :key="index" >
          <div class="con-left">
            <div class="con-left-head" >{{item.name}}<span>&bull;</span>7小时前<span>&bull;</span>{{item.type}}</div>
...
export default{
    ...
    data(){
      return{
        ...
        //搜索
        tabSearch:{show:true,func:'getSearchData'},
        //参数
        dataList:[],
        //广告列表
        adList:[
          {img:require('../../../assets/img/ad/01.jpg'),url:''},
          {img:require('../../../assets/img/ad/02.jpg'),url:''}
        ]
      }
    },
    methods:{
      ...
      getData(){
        const params = {}
        this.$Request.requestPost(params,this.$api.apiCommunity.list).then((res) => {
          console.log(res)
      ...
      // get search data
      getSearchData(param1) {
        const params = {
          title: param1,
        };
        this.$Request
          .requestPost(params, this.$api.apiCommunity.article_search)
          .then((res) => {
            this.$store.commit('communitySearchDataGet',res.data.data)
            if(this.$route.path == '/community'){
              this.$router.push({path:'community/search_result'})
            }else{
              this.getData();
            }
            console.log(res);
